...
export type CredentialRequirement = {
  env: string;
  /** Common legacy/env aliases that may already exist in a user's .env. */
  aliases?: string[];
  provider: string;
  label: string;
...
function googleWorkspaceCredentials(
  ...
  return [
    ...
    },
    {
      ...
      mint_hint:
        "connect.mjs mints this itself with a localhost loopback flow. Create a NEW Google " +
        "OAuth client of type 'Desktop app' first; a Web client will not work with this " +
        "loopback redirect. Fallback: copy the consent link or paste an existing refresh " +
        (calendarComponents.length > 0
...
export function buildCredentialManifest(
  ...
  // provider covers every model-backed step. Callers must choose explicitly:
  // no silent provider default is allowed.
